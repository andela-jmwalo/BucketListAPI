...
class TestBucketList(BaseTest):
    ...
    def test_get_bucketlists(self):
        ...
        response = self.client.get(
            '/bucketlists/', headers={"Authorization": self.retrieve_token(), 'Content-Type': 'application/json'})
        self.assertEqual(response.status_code, 200)

    def test_get_bucketlist_with_id(self):
        response = self.client.get(
            '/bucketlists/1', headers={"Authorization": self.retrieve_token(), 'Content-Type': 'application/json'})
        self.assertEqual(response.status_code, 200)
    ...
    def test_get_bucketlist_with_nonexistent_id(self):
        ...
        response = self.client.get(
            '/bucketlists/500', headers={"Authorization": self.retrieve_token(), 'Content-Type': 'application/json'})
        self.assertEqual(response.status_code, 400)
    ...
    def test_update_bucketlist(self):
        ...
        response = self.client.put(
            '/bucketlists/1', data=json.dumps(bucketlist1), headers={"Authorization": self.retrieve_token(), 'Content-Type': 'application/json'})
        self.assertEqual(response.status_code, 200)
